# Lýsing á verkefni
## Keyrla á verkefninu :
farið í command prompt og sækja þetta repositor. Síðan keyra eftirfarandi skipanir :
```sh
 *npm install
 *npm run dev
```
...
## Skipulag verkefnis
Forsíða verkefnisins er í skjalinu index.html. Allar aðrar síður eru fyrirlestur.html, einnig sem er geymd í almennu möppu.
...
Nokkrar sameiginlegar stillingar t.d max-width, gutter á grid er í config.scss skránni.

Allar myndir sem eru notaðar í verkefninu eru geymdar í möppunni img.
## Upplýsingar um þá sem unnu verkefnið
* Valdimar Orri Einarsson (voe3@hi.is)  --- GLEÐILEG JÓL :) 
